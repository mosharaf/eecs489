...
Before you start doing anything with this project, however, please [register your github username with us](https://goo.gl/forms/g8WVywnRCCPbNggm2) if you have not done so already. This is so that we can create a private repository for you to store your code and answers for this project.
...
**Note**: A good resource and a starting point to learn about socket programming is [Sockets Tutorial](http://www.cs.rpi.edu/~moorthy/Courses/os98/Pgms/socket.html). You may also refer to [Beej's Guide to Network Programming Using Internet Sockets](http://beej.us/net2/bgnet.html).
...
We will provide you an [autograder](#autograder) for Part 1 roughly halfway towards the deadline of this assignment. You should start testing your code early instead of waiting for the autograder to be posted. It is not intended to be used for debugging purposes.
...
You will be using our VM image ([link here](https://drive.google.com/file/d/1JNOH995lIz453AMdkVPtnDaJ0Wx0Oe5u/view?usp=sharing)) with Mininet 2.2.2 pre-installed. Please download and import the VM image into VirtualBox.

When you boot up the VM, you will have to log in using `mininet` and `mininet` as username and password.

Alternatively, if you have trouble with the method above, you can also set up a local VM with a recent version of Ubuntu (we did not run into any problems using Ubuntu 14.04) and follow options 2 or 3 listed [here](http://mininet.org/download/#option-2-native-installation-from-source).
...
To transfer files to/from your VM you can use the Shared Folder feature provided in VirtualBox.
...
We will provide you an autograder to evaluate your `iPerfer` in Part 1 roughly halfway towards the deadline of this assignment. The autograder will be the one we use to grade your `iPerfer`. It can be found inside the `autograder` folder under the assignment 1 folder once we post it. There will also be a README on how to use the autograder. We will make an anouncement on Piazza once the autograder is out.

Unlike other courses you may have taken here, you will have unlimited access to it. However, you are not encouraged to use it as a debugging tool to test your code.

Keep in mind that when we use the autograder to grade your assignment, we will only run it once. So you are responsible for making your `iPerfer` perform stable measurements.
